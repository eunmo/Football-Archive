...
module.exports = function(router, db) {
  ...
  router.get('/api/competition/select/:_season/:_compUrl', function(req, res) {
    ...
    function getLeague() {
      return Leagues.findOne({ season: season, name: comp }).then(function(
        league
      ) {
    ...
    function getCup() {
      return Cups.findOne({ season: season, name: comp }, { teams: 0 }).then(
        function(cup) {
          result.cup = cup;
        }
      );
    ...
    function getQual() {
      return Quals.findOne({ season: season, name: comp }, { teams: 0 }).then(
        function(qual) {
          result.qual = qual;
        }
      );
    ...
    function getGoals() {
      return Goals.findOne({ season: season, name: comp }, { teams: 0 }).then(
        function(goals) {
          if (goals !== null) {
