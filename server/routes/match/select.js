...
module.exports = function(router, db) {
  ...
  router.get('/api/match/select/:_url', function(req, res) {
    ...
    function getMatch(result) {
      return Matches.findOne({ url: url }).then(function(match) {
        if (match) {
    ...
    function getSeason(result) {
      return Seasons.findOne({ 'competitions.matches.url': url }).then(function(
        season
      ) {
        var i, comp;
        var j, match;
        ...
        for (i in season.competitions) {
          ...
          for (j in comp.matches) {
            ...
            if (match.url === url) {
              ...
              if (match.place === 'A') result.teams.reverse();
              return;
            }
          }
        }
