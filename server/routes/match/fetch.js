...
module.exports = function(router, db) {
  ...
  function getKLeagueMatch(url) {
    ...
    return KLeagueMatch.fetch(uri).then(summary => {
  ...
  function getJECupMatch(url) {
    return JECupMatch.fetch(url).then(summary => {
      if (summary === null) return;
  ...
  function formatKFACupMatch(data) {
    ...
    if (teamNormalizeNameMap[match.l]) match.l = teamNormalizeNameMap[match.l];

    if (teamNormalizeNameMap[match.r]) match.r = teamNormalizeNameMap[match.r];
    ...
    function addAssist(row, player) {
      if (row.assists === undefined) return;
    ...
    for (i = 0; i < match.goals.length; i++) {
      ...
      if (goal.style !== undefined) continue;

      if (assistMap[goal.minute]) goal.assist = assistMap[goal.minute];
    }
  ...
  function getKFACupMatch(url) {
    ...
    const execStr =
      'perl ' +
      path.join(__dirname, '../../../perl', 'kfacup_match.pl') +
      ' ' +
      uri;
    ...
    return exec(execStr).then(function(data) {
      ...
      if (data.starting[0].length === 0) return;
  ...
  function fetchMatchUrl(url) {
    if (url === '' || url === undefined || url === 'undefined') return;

    if (url.match(/^KL/)) return getKLeagueMatch(url);

    if (url.match(/^KFACUP/)) return getKFACupMatch(url);

    if (url.match(/^JECUP/)) return getJECupMatch(url);

    const execStr =
      'perl ' + path.join(__dirname, '../../../perl', 'match.pl') + ' ' + url;
    ...
    return exec(execStr)
      .then(function(data) {
        ...
        if (teamNameMap[data.l]) data.l = teamNameMap[data.l];

        if (teamNameMap[data.r]) data.r = teamNameMap[data.r];
  ...
  function fetchThenRespond(res, urls) {
    Matches.find({ url: { $in: urls } }, { _id: 0, url: 1 })
      .toArray()
