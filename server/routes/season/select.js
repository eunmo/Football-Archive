...
module.exports = function(router, db) {
  ...
  router.get('/api/season/select/:_season/:_teamUrl', function(req, res) {
    ...
    function getMatches(urls, map) {
      return Matches.find({ url: { $in: urls } })
        .toArray()
    ...
    function getLeagueTables(season, team, obj) {
      return Leagues.find({ season: season, 'table.name': team })
        .toArray()
    ...
    function getCups(season, team, obj) {
      return Cups.find({ season: season, teams: team }, { teams: 0 })
        .toArray()
        .then(function(cups) {
          obj.cups = cups;
        });
    ...
    function getQuals(season, team, obj) {
      return Quals.find({ season: season, teams: team }, { teams: 0 })
        .toArray()
        .then(function(quals) {
          obj.quals = quals;
        });
  ...
  router.get('/api/season/select/:_season', function(req, res) {
    ...
    Seasons.find({ season: season }, { _id: 0, team: 1 })
      .toArray()
      .then(function(seasons) {
