...
module.exports = function(router, db) {
  ...
  router.get('/api/history/team/:_teamUrl', function(req, res) {
    ...
    function getLeague() {
      return Leagues.find({ 'table.name': team })
        .toArray()
        .then(leagues => {
          var i, row;
          var result = [];
  ...
  router.get('/api/history/competition/:_compUrl', function(req, res) {
    ...
    function getLeague() {
      return Leagues.find({ name: comp })
        .toArray()
        .then(leagues => {
          if (leagues.length > 0)
            result.leagues = leagues.sort(function(a, b) {
              return b.season - a.season;
            });
        });
    }
      ...
      return Cups.find({ name: comp })
        ...
        .then(cups => {
          ...
          for (i = 0; i < cups.length; i++) {
            ...
            if (entry.rounds.length > 0) results.push(entry);
          }

          if (results.length > 0)
            result.cups = results.sort(function(a, b) {
              return b.season - a.season;
            });
        });
    }
