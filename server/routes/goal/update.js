...
module.exports = function(router, db) {
  ...
  function getLeagues(season) {
    return Leagues.find({ season: season }, { _id: 0, name: 1 })
      .toArray()
      .then(function(data) {
        var array = ['World Cup'];
  ...
  function normalizeNames(season, leagueName, cupName, urlMap) {
    ...
    if (league === undefined || cup === undefined) return;
    ...
    Promise.all(promises).then(async function(array) {
      ...
      for (l in leagues) {
        ...
        if (league.goals.length > 0) {
          bulk
            .find({ season: league.season, name: league.name })
            .upsert()
            .update({ $set: { goals: league.goals } });
          hasBulkOp = true;
        }
      }
