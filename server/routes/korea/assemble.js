...
module.exports = function(router, db) {
  ...
  function formatRound(season, league, gameid) {
    const compMap = {
      1: 'K League 1',
      2: 'K League 2',
      4: 'K League Relegation'
    };
    var round = gameid;

    if (league === 4) return [compMap[league], 'PO'];
    ...
    if (league === 1) {
      ...
      } else if (false || season === '2015' || season === '2016') {
        if (gameid > 44 * 5) {
          league = 4;
          round = gameid -= 44 * 5;
        } else {
  ...
  router.get('/api/korea/assemble/:_season', function(req, res) {
    ...
    promises.push(
      Cups.findOne({ season: season, name: 'AFC Champions League' })
    );

    Promise.all(promises).then(async function(array) {
      ...
      for (i in league.games) {
        ...
        getComp(teamMap[match.home], comp, season).matches.push({
          date: match.date,
          place: 'H',
          round: round,
          vs: match.away,
        ...
        getComp(teamMap[match.away], comp, season).matches.push({
          date: match.date,
          place: 'A',
          round: round,
          vs: match.home,
      ...
      for (i in cup.rounds) {
        ...
        for (j in round.matches) {
          ...
          if (teamMap[match.l]) {
            ...
            if (match.url !== undefined) game.url = match.url;
      ...
      if (aclTeams[season]) {
        if (acl === null) {
          comp = {
            ...
            url: '/all_matches/afc-champions-league-' + season + '/',
            matches: []
          };
          ...
          for (i = 0; i < aclTeams[season].length; i++) {
            ...
            for (j = 0; j < acl.rounds.length; j++) {
              ...
              for (k = 0; k < round.matches.length; k++) {
                ...
                if (!(match.l === team || match.r === team)) continue;
      ...
      for (i in teamMap) {
        team = teamMap[i];
        bulk
          .find({ season: team.season, team: team.team })
          .upsert()
          .update({
            $set: { assembled: true, competitions: team.competitions }
          });
      }
