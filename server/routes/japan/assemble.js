...
module.exports = function(router, db) {
  ...
  router.get('/api/japan/assemble/:_season', async function(req, res) {
    ...
    var seasons = await Seasons.find({
      season: season,
      team: { $in: cup.teams }
    }).toArray();
    var teamMap = {};
    ...
    seasons.forEach(season => {
      ...
      season.competitions = season.competitions.filter(
        c => c.name !== "Emperor's Cup"
      );
    });
    ...
    cup.rounds.forEach(round => {
      round.matches.forEach(match => {
        if (teamMap[match.l]) {
          ...
          if (round.name === 'Final') game.place = 'N';
    ...
    for (var i in teamMap) {
      ...
      bulk
        ...
        .upsert()
        .update({ $set: { competitions: team.competitions } });
    }
