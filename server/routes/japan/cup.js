...
module.exports = function(router, db) {
  ...
  const Cups = db.collection('Cups');
	...
	function fetchCup(year, teamMap) {
		var url = 'http://www.jfa.jp/match/emperorscup_2014/2014/match/schedule.json';
		...
		return get(url).then(data => {
			var cup = {name: "Emperor's Cup", season: year, rounds: [], assembled: true};
			var rounds = cup.rounds;
			var teams = {};

			data.matchScheduleList.matchSchedule.forEach(match => {
				if (match.matchDate === '未定')
					return;
				...
				if (teamNameMap[newMatch.l])
					newMatch.l = teamNameMap[newMatch.l];

				if (teamNameMap[newMatch.r])
					newMatch.r = teamNameMap[newMatch.r];
				...
				if (match.score.homePKScore !== undefined && match.score.awayPKScore !== undefined)
