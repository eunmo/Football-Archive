...
const teamNameMap = KLeagueUtil.leagueMatchTeamNameMap;
...
function getPlayers(array, side) {
  ...
  array.forEach(player => {
    result.push({
      number: player[side + '_Back_No'],
      name: player[side + '_Player_Name']
    });
  });
...
function getPlayerMap(players) {
  ...
  players.start.forEach(player => {
    map[player.number] = player;
  });
  players.sub.forEach(player => {
    map[player.number] = player;
  });
...
function addSub(player, minute) {
  ...
  if (player.sub !== undefined) player.sub = [player.sub, minute];
  else player.sub = minute;
}
...
module.exports = {
  fetch: function(url) {
    const execStr =
      'perl ' +
      path.join(__dirname, '../../perl', 'kleague_match.pl') +
      " '" +
      url +
      "'";
    ...
    return exec(execStr).then(function(data) {
      ...
      if (match.l === null || match.r === null) return null;
