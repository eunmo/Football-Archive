...
module.exports = {
  fetch: function(season, team) {
    ...
    function updateSeason(season, team) {
      ...
      const execStr =
        'perl ' +
        path.join(__dirname, '../../perl', 'season.pl') +
        ' ' +
        season +
        ' ' +
        teamUrl;
    ...
    if (jleague.includes(team)) {
      var promises = [
        updateSeason(season, team),
        updateSeason(season - -1, team)
      ];
      return Promise.all(promises).then(function(array) {
        ...
        for (i in array) {
          ...
          for (j in year) {
            ...
            if (competition.name === 'J') continue;
              ...
              if (competition.name.match(/^J1 League.*Phase/)) {
                if (competition.name.match(/2. Phase$/)) {
                  for (k in competition.matches) {
                    match = competition.matches[k];
                    match.round =
                      parseInt(match.round.replace(' Round')) + 17 + ' Round';
                  }
                }
                ...
                if (found === false) newSeason.competitions.push(competition);
              }
            }
          }
        }

        if (jleague.matches.length > 0) newSeason.competitions.push(jleague);
      ...
      return updateSeason(season, team).then(function(data) {
        var newSeason = {
          season: season,
          team: team,
          competitions: data
