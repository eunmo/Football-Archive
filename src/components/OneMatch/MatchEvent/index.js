...
export default class MatchEvent extends Component {
  render() {
    ...
    var minute = (
      <div className="MatchEvent-minute text-center">{this.props.minute}</div>
    );
    var type = null;
    var detail = null;
      ...
      <div className="MatchEvent-player">
        ...
        {goal.assist && (
          <div className="MatchEvent-assist">
            assist by {PlayerName.getDisplayName(goal.assist)}
          </div>
        )}
      </div>
    );
  ...
  renderSub() {
    ...
    const minute = (
      <div className="MatchEvent-minute text-center MatchEvent-sub">
        {this.props.minute}
      </div>
    );
    const type = (
      <div className="MatchEvent-inout text-center MatchEvent-sub">
        <div className="MatchEvent-scorer">{side === 'r' ? '◀' : '▶'}</div>
        <div className="MatchEvent-assist">{side === 'l' ? '◀' : '▶'}</div>
      </div>
    );
    const detail = (
