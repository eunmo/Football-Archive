...
export default class CompetitionView extends Component {
  ...
  render() {
    ...
    const basename =
      '/competition/' + this.state.year + '/' + this.state.compUrl;
        ...
        <div className="flex-container text-center">
          ...
          <div className="flex-2">
            <div
              ...
              className="text-center CompetitionView-view-selector"
            >
  ...
  getHistoryLink() {
    if (this.state.data.qual) return null;

    return (
      <Link to={'/history/competition/' + UrlUtil.getCompUrl(this.state.name)}>
        <div className="CompetitionView-view-selector">History</div>
      </Link>
    );
  }
  ...
  fetchNationsLeague(year) {
    ...
    leagues.forEach(league => {
      ...
      var url = UrlUtil.getCompetitionSelectUrl(
        compYear,
        UrlUtil.getCompUrl(compName)
      );
    ...
    Promise.all(promises).then(dataArray => {
      if (dataArray[0].name === undefined) return;
