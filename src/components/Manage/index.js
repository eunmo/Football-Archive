...
export default class Manage extends Component {
    ...
    return (
      <div className="Manage">
        <h2 className="text-center">
          {this.state.selectedCountry.toUpperCase()} {this.state.selectedYear}{' '}
          Season
        </h2>
                  ...
                  <ul>
                    {entry.years.map(year => {
                      return (
                        <li
                          key={year}
                          onClick={() => this.selectYear(entry.title, year)}
                        >
                          {year}
                        </li>
                      );
          ...
          <div className="flex-2">
            <div className="flex-container Manage-team">
              <div className="flex-1">
                ...
                <button onClick={() => this.updateCup()}>Update Cup</button>
                ...
                <button onClick={() => this.updateGoals()}>Update Goals</button>
              </div>
            </div>
                ...
                <div className="flex-container Manage-team" key={team}>
                  ...
                  <div className="flex-1">
                    {this.state.fetchedTeams[team] ? (
                      <button
                        className="Manage-clear-btn"
                        onClick={() => this.clearSeason(team)}
                      >
                        Clear Season
                      </button>
                    ) : (
  ...
  markDoneAllSeasons() {
    ...
    for (i = 0; i < teams.length; i++) {
      team = teams[i];
      if (
        this.state.fetchedTeams[team] === true &&
  ...
  updateLeague() {
    ...
    fetch(url, { leagues: leagues }).then(function(response) {
