...
export default class TeamHistory extends Component {
          ...
          return (
            ...
            >
              <div className="flex-1" style={seasonStyle}>
                {this.getSeasonSpan(season.year)}
              </div>
              {season.competitions.map((competition, index) => {
                var link = null;
                if (competition !== null)
                  link = UrlUtil.getCompLink(
                    competition.season,
                    competition.name
                  );
  ...
  getLeagueView(competition, link) {
    if (competition === null || competition.league === undefined) return null;

    const rank = competition.league.rank;
    var text = rank === 1 ? this.getTrophy() : rank;

    if (link) text = <Link to={link}>{text}</Link>;

    return text;
  }

  getCupView(competition, link) {
    if (competition === null || competition.cup === undefined) return null;
    ...
    if (cup.table) {
      for (i = 0; i < cup.table.length; i++) {
        ...
        if (row.name === this.state.team) {
          ...
          if (link) result = <Link to={link}>{result}</Link>;
    ...
    const vs =
      cup.matches[0].l === this.state.team
        ? cup.matches[0].r
        : cup.matches[0].l;
    ...
    if (result === '3') {
      ...
      if (sum.resultFull.includes('loss')) result = '4';
    }

    if (cup.winner === this.state.team) result = this.getTrophy();
