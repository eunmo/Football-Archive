...
export default class Versus extends Component {
  ...
  render() {
    ...
    return (
      <div>
        ...
        {this.state.seasons.map(season => {
          return (
            <div key={season.year} className="flex-container">
              <div className="flex-1 text-center" style={rowStyle}>
              ...
              {season.competitions.map((competition, index) => {
                ...
                if (comp.type === 'H') scoreboard1 = null;
        ...
        <div className="text-center">
          <Link
            to={'/versus/' + this.state.teamBUrl + '/' + this.state.teamAUrl}
          >
            swap left & right
          </Link>
        </div>
  ...
  getSeasonSpan(year) {
    var fullyear =
      this.state.teamAUrl.endsWith('-team') ||
  ...
  groupMatches(data) {
    ...
    if (matches.length === 0) return { teamA: data.teamA, teamB: data.teamB };
    ...
    for (i = 0; i < matches.length; i++) {
      ...
      if (comp.type === 'H') matchIndex = 0;

      if (
        comp.type === '4' &&
        seasonMap[match.season].competitions[compIndex][matchIndex] !==
          undefined
      )
        matchIndex += 2;
