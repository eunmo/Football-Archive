...
export default class Circle extends Component {
  ...
  render() {
    ...
    function getPath(url, key, d, stroke) {
      var path = (
        <path key={key} d={d} stroke={stroke} fill="none" strokeWidth="20" />
      );

      if (url) {
        path = (
          <Link key={key} to={'/match/' + url}>
            {path}
          </Link>
        );
      }
  ...
  playerMinutes(match) {
    ...
    if (summary === undefined || summary.players === undefined) return null;
    ...
    for (var i = 0; i < players.start.length; i++) {
      ...
      if (entry.name === name) {
        if (
          entry.card &&
          (entry.card.type === 'red' || entry.card.type === 'Second yellow')
        ) {
  ...
  playerGoals(match) {
    ...
    if (summary === undefined || summary.goals === undefined) return count;
