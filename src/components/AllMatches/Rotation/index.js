...
export default class Rotation extends Component {
  render() {
    ...
    if (this.props.data.player) {
      squad.appearances = squad.appearances.filter(
        p => p.name === this.props.data.player.fullname
      );
    }
        ...
        <div className="flex-container-right-aligned">
          {playedMatches.map((match, index) => (
            <Scoreboard
              key={index}
              team={team}
              match={{ date: match.date, url: match.url }}
            />
          ))}
        </div>
        <div className="flex-container">
        ...
        {squad.appearances.map(player => {
          ...
          return [
            ...
            <div key={'player'} style={{ marginLeft: '10px' }}>
              {PlayerNameUtil.capitalize(
                PlayerNameUtil.divide(player.name).last
              )}
            </div>
          ];
        })}
