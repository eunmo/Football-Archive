...
export default class Form extends Component {
  ...
  getGoals(match) {
    if (match.summary === undefined) return null;
    ...
    return match.summary.goals.map((goal, index) => {
      ...
      if (side === 'l') {
        ...
        if (
          ...
          goal.scorer === this.props.data.player.fullname
        )
          scorerStyle.fontWeight = 'bold';

        array.push(
          <div key={index + 'scorer'} style={scorerStyle}>
            {goal.scorer} {scoreType}
          </div>
        );
      }
      ...
      if (side === 'r') style.textAlign = 'right';
      ...
      if (side === 'l' && goal.assist) {
        ...
        if (
          this.props.data.player &&
          goal.assist === this.props.data.player.fullname
        )
          assistStyle.fontWeight = 'bold';

        array.push(
          <div key={index + 'assist'}>
            (A) <span style={assistStyle}>{goal.assist}</span>
          </div>
        );
      }
  ...
  getMatchesView(matches) {
    return matches.map((match, index) => {
      return this.getMatchView(match, index);
    });
  }
}
