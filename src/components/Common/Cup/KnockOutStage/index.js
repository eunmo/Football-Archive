...
export default class KnockOutStage extends Component {
  render() {
    ...
    for (i = 0; i < rounds.length; i++) {
      ...
      if (i === lastRoundIndex) {
        if (i === 0) {
          ...
          if (cup.winner)
            color =
              cup.winner === this.props.team
                ? colors.mediumblue
                : colors.mediumred;

          lastRound = (
            <circle key={0} cx={cx} cy={cy} r={r + hsize + 1} fill={color} />
          );
        } else {
          lastRound = (
            <circle
      ...
      for (j = 0; j < round.grid.length; j++) {
        ...
        if (
          (i === 0 && (log2 > 1 || cup.winner === undefined)) ||
        ...
        image = (
          <image
            key={log2 + team}
            xlinkHref={UrlUtil.getEmblemUrl(team)}
            x={x}
            y={y}
            width={size}
            height={size}
    ...
    if (cup.winner) {
      image = (
        <image
          xlinkHref={UrlUtil.getEmblemUrl(cup.winner)}
          x={cx - hwsize}
          y={cy - hwsize}
          width={wsize}
          height={wsize}
