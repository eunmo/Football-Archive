...
export default class Progress extends Component {
        ...
        {rows.map((row, index) => {
          ...
          return (
            <div className="flex-container" style={style} key={index}>
              <div
                className="flex-1 flex-container-right-aligned Progress-left"
                style={innerStyle}
              >
              ...
              {row.view
                ...
                : row.matches.map((match, index) => {
                    if (match === null)
                      return (
                        <div key={index} className="Progress-empty-match" />
                      );
  ...
  buildLeagueRows() {
    ...
    if (leagueTable === undefined) return [];
    ...
    for (i = 0; i < leagueTable.length; i++) {
      ...
      if (entry.name === team) {
        selfview = (
          <div className="Progress-self">{this.getRankSuffix(entry.rank)}</div>
        );
  ...
  buildQualifierRows() {
    ...
    for (i = qual.rounds.length - 1; i >= 0; i--) {
      ...
      if (roundNameMap[round.name] === undefined) continue;
      ...
      if (round.table) {
        ...
        table[tableIndex++] = {
          team: vs,
          round: round.name,
          view: this.getFinalView(round.matches[0])
        };
      } else {
        teamMap = {};
        teams = [];
  ...
  formatGroupStage(rows, width) {
    ...
    if (this.props.cup === undefined) return rows;
    ...
    if (roundName === undefined) return rows;
