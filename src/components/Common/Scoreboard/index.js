...
export default class Scoreboard extends Component {
  render() {
    ...
    var inner = [
      bg,
      <div key={1} className="Scoreboard-inner">
        {score}
      </div>
    ];

    if (match.url === undefined) {
      if (result !== 'unplayed') style.color = 'white';
    ...
    for (var i = 0; i < players.start.length; i++) {
      if (players.start[i].name === player) {
        if (players.start[i].sub) {
          return (
            <div key={0} className="Scoreboard-out" style={styleOut}></div>
          );
        } else {
          return null;
        }
  ...
  playerScored(goals, side, player) {
    ...
    if (this.props.skipScore) return null;
    ...
    for (var i = 0; i < goals.length; i++) {
      ...
      if (goal.scorer === player) goalCount++;

      if (goal.assist === player) assistCount++;
    }
